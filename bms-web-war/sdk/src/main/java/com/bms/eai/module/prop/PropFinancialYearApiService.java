...
import com.bms.eai.module.prop.beans.PropFinanceYear;
 ...
 */
@Service
public class PropFinancialYearApiService extends AbstractPropDetailsRestApiClient<PropFinanceYear, JsonResponseBean> {

	private SdkBroker preCheckData(String screenFlag) throws JsonParseException, JsonMappingException, IOException {

		final Optional<PfyOperations> operations = super.propertyClientApiAccess.getPropertyFinancialYearOperations();

		if (!operations.isPresent()) {
			return new SdkBroker(this.generateJsonMsg(this.unknownSchemaConfig(ApiConstants.PROP_FINANCIAL_YEAR)));
		}
	...
	private JsonResponseBean financialYearOperations(String updatePropDetailsMasterId, String screenFlag,
			PropFinanceYear pdm) throws ResourceAccessException, Exception {
		...
		urlBuilder.append(FINANCIALYEAR_RESOURCE_PATH).append(HTTP_SLASH);
		...
		if (StringUtils.hasText(screenFlag) && StringUtils.pathEquals(screenFlag, CREATE)) {
			...
			logger.info("[CREATE {}, RESOURCE URL : {}]",
					new Object[] { FINANCIALYEAR_RESOURCE_PATH, urlBuilder.toString() });
		...
		logger.info("[{}, Result JsonResponseBean :{}]", new Object[] { FINANCIALYEAR_RESOURCE_PATH, jrb });
		return jrb;
	}

	public ResponseEntity<JsonNode> processFinancialYearOperations(final PropFinanceYear propDetailsMaster,
		...
		if (StringUtils.hasText(jrbean.getStatusCode())
			...
			jrbean = null;
			jrbean = this.financialYearOperations(updatePropDetailsMasterId, screenFlag, propDetailsMaster);
