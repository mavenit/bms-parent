 ...
 */
@Service
public class CommonStateApiService extends AbstractCommonRestApiClient<CommonBean,JsonResponseBean> implements CommonDataAccess<CommonBean> {
	...
	private JsonResponseBean processOperations(String id, String screenFlag,CommonBean reqJsonBean) throws ResourceAccessException, Exception {
		...
		}else if(StringUtils.hasText(screenFlag) && StringUtils.pathEquals(screenFlag, LOADBYCOUNTRY)) {
			urlBuilder.append(LOADBYCOUNTRY).append(HTTP_SLASH).append(id);
			logger.info("[LOADBYNAME {}, RESOURCE URL : {}]", new Object[] {COUNTRY_RESOURCE_PATH,urlBuilder.toString() });
			jrb = super.execute(false,new RequestDetails(urlBuilder.toString(), HttpMethod.GET, id),
					null, super.generateResponseHandler(), JsonResponseBean.class);
		}
	...
	public JsonResponseBean loadByCountry(String countryId) throws ResourceAccessException, Exception {
		logger.info("SDK Load By Country ......");
		return this.processOperations(countryId,LOADBYCOUNTRY,null);
	}
