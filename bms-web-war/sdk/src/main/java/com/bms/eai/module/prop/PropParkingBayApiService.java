...
import com.bms.eai.module.prop.beans.PropParkingBayDetails;
 ...
 */
@Service
public class PropParkingBayApiService
		extends AbstractPropDetailsRestApiClient<PropParkingBayDetails, JsonResponseBean> {

	private SdkBroker preCheckData(String screenFlag) throws JsonParseException, JsonMappingException, IOException {

		final Optional<PpbsOperations> operations = super.propertyClientApiAccess.getPropertyParkingBayOperations();

		if (!operations.isPresent()) {
			return new SdkBroker(this.generateJsonMsg(this.unknownSchemaConfig(ApiConstants.PROP_BAY_PARKING)));
		}
	...
	private JsonResponseBean parkingBayOperations(String updatePropDetailsMasterId, String screenFlag,
			PropParkingBayDetails pdm) throws ResourceAccessException, Exception {
		...
		urlBuilder.append(BAYPARKING_RESOURCE_PATH).append(HTTP_SLASH);
		...
		if (StringUtils.hasText(screenFlag) && StringUtils.pathEquals(screenFlag, CREATE)) {
			...
			logger.info("[CREATE {}, RESOURCE URL : {}]",
					new Object[] { BAYPARKING_RESOURCE_PATH, urlBuilder.toString() });
		...
		logger.info("[{}, Result JsonResponseBean :{}]", new Object[] { BAYPARKING_RESOURCE_PATH, jrb });
		return jrb;
	}

	public ResponseEntity<JsonNode> processParkingBayOperations(
			final PropParkingBayDetails propDetailsMaster, final String updatePropDetailsMasterId,
		...
		if (StringUtils.hasText(jrbean.getStatusCode())
			...
			jrbean = null;
			jrbean = this.parkingBayOperations(updatePropDetailsMasterId, screenFlag, propDetailsMaster);
