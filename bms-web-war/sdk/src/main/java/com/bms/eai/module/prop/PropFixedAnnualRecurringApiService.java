...
import com.bms.eai.module.prop.beans.PropFixedAnualRecurring;
 ...
 */
@Service
public class PropFixedAnnualRecurringApiService
		extends AbstractPropDetailsRestApiClient<PropFixedAnualRecurring, JsonResponseBean> {

	private SdkBroker preCheckData(String screenFlag) throws JsonParseException, JsonMappingException, IOException {

		final Optional<PfarOperations> operations = super.propertyClientApiAccess
				.getPropertyFixedAnnualRecurringOperations();

		if (!operations.isPresent()) {
			return new SdkBroker(this.generateJsonMsg(this.unknownSchemaConfig(ApiConstants.PROP_FIXED_ANNUAL_RECURRING)));
		}
	...
	private JsonResponseBean fixedAnnualRecurringOperations(String updatePropDetailsMasterId, String screenFlag,
			PropFixedAnualRecurring pdm) throws ResourceAccessException, Exception {
		...
		urlBuilder.append(FIXEDANNUALRECURRING_RESOURCE_PATH).append(HTTP_SLASH);

		JsonResponseBean jrb = null;

		if (StringUtils.hasText(screenFlag) && StringUtils.pathEquals(screenFlag, CREATE)) {
			urlBuilder.append(CREATE);
			logger.info("[CREATE {}, RESOURCE URL : {}]",
					new Object[] { FIXEDANNUALRECURRING_RESOURCE_PATH, urlBuilder.toString() });
			...
			logger.info("[UPDATE {}, RESOURCE URL : {}]",
					new Object[] { FIXEDANNUALRECURRING_RESOURCE_PATH, urlBuilder.toString() });
			...
			logger.info("[DELETE {}, RESOURCE URL : {}]",
					new Object[] { FIXEDANNUALRECURRING_RESOURCE_PATH, urlBuilder.toString() });
		...
		logger.info("[{}, Result JsonResponseBean :{}]", new Object[] { FIXEDANNUALRECURRING_RESOURCE_PATH, jrb });
		return jrb;
	}

	public ResponseEntity<JsonNode> processFixedAnnualRecurringOperations(final PropFixedAnualRecurring propDetailsMaster,
		...
		if (StringUtils.hasText(jrbean.getStatusCode())
			...
			jrbean = null;
			jrbean = this.fixedAnnualRecurringOperations(updatePropDetailsMasterId, screenFlag, propDetailsMaster);
